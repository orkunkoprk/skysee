...
if (@$_GET['urunsil']=="ok") {
	...
	$sil=$db->prepare("DELETE from urun where ucak_id=:ucak_id");
	$kontrol=$sil->execute(array(
...
if (isset($_POST['urunekle'])) {
	...
	$ucak_seourl=seo($_POST['ucak_marka']);

	$kaydet=$db->prepare("INSERT INTO urun SET
		kategori_id=:kategori_id,
		ucak_marka=:ucak_marka,
		ucak_detay=:ucak_detay,
		bilet_fiyat=:bilet_fiyat,
		ucak_video=:ucak_video,
		ucak_keyword=:ucak_keyword,
		ucak_durum=:ucak_durum,
		ucak_gidis=:ucak_gidis,
		ucak_donus=:ucak_donus,
		bilet_stok=:bilet_stok,	
		ucak_seourl=:seourl		
		");
	$insert=$kaydet->execute(array(
		'kategori_id' => $_POST['kategori_id'],
		'ucak_marka' => $_POST['ucak_marka'],
		'ucak_detay' => $_POST['ucak_detay'],
		'bilet_fiyat' => $_POST['bilet_fiyat'],
		'ucak_video' => $_POST['ucak_video'],
		'ucak_keyword' => $_POST['ucak_keyword'],
		'ucak_durum' => $_POST['ucak_durum'],
		'bilet_stok' => $_POST['bilet_stok'],
		'ucak_gidis' => $_POST['ucak_gidis'],
		'ucak_donus' => $_POST['ucak_donus'],
		'seourl' => $ucak_seourl
...
if (isset($_POST['urunduzenle'])) {
	...
	$update=$kaydet->execute(array(
		...
		'bilet_fiyat' => $_POST['bilet_fiyat'],
		'ucak_video' => $_POST['ucak_video'],
		'ucak_onecikar' => $_POST['ucak_onecikar'],
		'ucak_keyword' => $_POST['ucak_keyword'],
		'ucak_durum' => $_POST['ucak_durum'],
		'bilet_stok' => $_POST['bilet_stok'],
		'ucak_gidis' => $_POST['ucak_gidis'],
	...
	if ($update) {
		...
		Header("Location:../production/urun-duzenle.php?durum=ok&ucak_id=$ucak_id");

	} else {

		Header("Location:../production/urun-duzenle.php?durum=no&ucak_id=$ucak_id");
	}
...
if (@$_GET['ucak_onecikar']=="ok") {
	...
	$duzenle=$db->prepare("UPDATE urun SET
		...
		WHERE ucak_id={$_GET['ucak_id']}");
	
	$update=$duzenle->execute(array(


		'ucak_onecikar' => $_GET['urun_one']
	));
...
if (isset($_POST['bankasiparisekle'])) {
	...
	$bilet_tip="Banka Havalesi";


	$kaydet=$db->prepare("INSERT INTO siparis SET
		kullanici_id=:kullanici_id,
		bilet_tip=:bilet_tip,	
		banka_turu=:banka_turu,
		bilet_toplam=:bilet_toplam,
		ucak_id =:ucak_id,
		ucak_gidis=:ucak_gidis,
		ucak_donus=:ucak_donus
		");
	$insert=$kaydet->execute(array(
		'kullanici_id' => $_POST['kullanici_id'],
		'bilet_tip' => $bilet_tip,
		'banka_turu' => $_POST['banka_turu'],
		'bilet_toplam' => $_POST['bilet_toplam'],
		'ucak_id' => $_POST['ucak_id'],
		'ucak_gidis' => $_POST['ucak_gidis'],
		'ucak_donus' => $_POST['ucak_donus']
	...
	if ($insert) {
		...
		while($sepetcek=$sepetsor->fetch(PDO::FETCH_ASSOC)) {
			...
			$bilet_adet=$sepetcek['bilet_adet'];
			...
			echo $bilet_fiyat=$uruncek['bilet_fiyat'];
...
if(isset($_POST['urunfotosil'])) {
	...
	foreach($checklist as $list) {
		...
		$sil=$db->prepare("DELETE from urunfoto where ucakfoto_id=:ucakfoto_id");
		$kontrol=$sil->execute(array(
			'ucakfoto_id' => $list
		));
	}
	...
	if ($kontrol) {
		...
		Header("Location:../production/urun-galeri.php?ucak_id=$ucak_id&durum=ok");

	} else {

		Header("Location:../production/urun-galeri.php?ucak_id=$ucak_id&durum=no");
	}
